...
-
-
  15. 补更的第四篇论文推荐是来自ISSTA 2021的一篇Tool-Demonstration 的论文MoScan: A Model-Based Vulnerability Scanner for Web Single Sign-On Services[好喜欢]许多web应用使用第三方的单点登录服务(single sign-on)来简化授权和认证过程，然而这些服务一直面临着安全问题[失望]单点登录服务允许用户使用注册在identify provider(IdP)的账号去登陆其他relying parties(RPs)[哼]RP可以通过IdP提供的token调用restful APIs来获取在IdP上的身份信息[哼]这些SSO服务大多建立在OAuth2.0和OpenID Connect标准上，但是IdPs和RPs可能在实现过程中不完全遵循这些标准，从而导致安全问题[并不简单]所以作者提出了基于黑盒测试的工具MoSCAN来确认SSO实现中的安全漏洞[阴险]